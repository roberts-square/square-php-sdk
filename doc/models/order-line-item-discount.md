...
| `discountCodeIds` | `?(string[])` | Optional | The discount code IDs corresponding to this discount. Either<br>`discount_code_ids` or `reward_ids` can be set for a discount, but not both. The application and<br>specification of discounts that have `discount_code_ids` are completely controlled by<br>the backing criteria corresponding to the discount codes when the discount codes are<br>added to the order through the Discount Codes API. To manually unapply discounts that<br>are the result of added discount codes, the discount codes must be removed from the order<br>through the Discount Codes API. | getDiscountCodeIds(): ?array | setDiscountCodeIds(?array discountCodeIds): void |
...
| `modifyTaxBasis` | [`?string (OrderLineItemDiscountModifyTaxBasis)`](../../doc/models/order-line-item-discount-modify-tax-basis.md) | Optional | Indicates whether the discount should reduce the price used to calculate tax. | getModifyTaxBasis(): ?string | setModifyTaxBasis(?string modifyTaxBasis): void |
| `applyPerQuantity` | `?bool` | Optional | Indicates whether the discount applies to every quantity of the itemization. | getApplyPerQuantity(): ?bool | setApplyPerQuantity(?bool applyPerQuantity): void |
| `applicationMethod` | [`?string (OrderLineItemDiscountApplicationMethod)`](../../doc/models/order-line-item-discount-application-method.md) | Optional | Categorizes why a discount was applied to an order or line item. | getApplicationMethod(): ?string | setApplicationMethod(?string applicationMethod): void |
