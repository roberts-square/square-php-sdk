...
| `legacyTaxIds` | `?(string[])` | Optional | fee tokens that item applied from legacy `ItemFeeMembership`. For internal and read only | getLegacyTaxIds(): ?array | setLegacyTaxIds(?array legacyTaxIds): void |
...
| `additionalCategories` | [`?(CatalogObjectCategory[])`](../../doc/models/catalog-object-category.md) | Optional | The list of additional categories | getAdditionalCategories(): ?array | setAdditionalCategories(?array additionalCategories): void |
